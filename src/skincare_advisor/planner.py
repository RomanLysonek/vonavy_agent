...
from skincare_advisor.adapters import adapter_capabilities
from skincare_advisor.domain import ExperimentSpec, JobState
...
def propose_experiments(
    ...
    optional_model = capabilities["optional_model"]
    baseline_wape = [
        metric.value
    ...
    if (
        optional_model["available"]
        and optional_model["supports_probability_calibration"]
        and baseline_wape
        and len(proposals) < 3
    ):
        proposals.append(
            {
                "rank": 0,
                "kind": "optional_model_challenger",
