...
from skincare_advisor.hashing import canonical_hash, canonical_json, file_hash
from skincare_advisor.identity import LOCAL_OWNER_ID
...
class CatalogRegistry:
    ...
    def _ingest_staged(
        ...
        *,
        ...
        if not catalog_name.strip() or len(catalog_name) > 200:
            raise AgentError("invalid_catalog_name", "Catalog name must contain 1-200 characters")
        if mode not in {"snapshot", "append"}:
            raise AgentError("invalid_ingest_mode", "Ingest mode must be snapshot or append")
        if mode == "append" and (not catalog_id or not parent_version_id):
            raise AgentError("missing_parent", "Append requires catalog_id and parent_version_id")
        ...
        if not len(frame.columns) or frame.columns.duplicated().any():
            raise AgentError("invalid_schema", "Catalog columns must be present and unique")
        source_blob = self._publish_blob(temp_path, media_type)
        materialized_blob, materialized_rows = self._materialize(
            frame, mode, catalog_id, parent_version_id, owner_id
        )
        with session_scope(self.engine) as session:
            catalog = self._resolve_catalog(session, catalog_name, catalog_id, mode, owner_id)
            version_number = (
                session.scalar(
                    select(func.coalesce(func.max(CatalogVersion.version_number), 0)).where(
                        CatalogVersion.catalog_id == catalog.id
                    )
                )
                or 0
            ) + 1
        ...
        with Session(self.engine) as session:
            return session.get_one(CatalogVersion, version_id)
    ...
    def _resolve_catalog(
        ...
        if catalog_id:
            ...
            if catalog is None or catalog.owner_id != owner_id:
                raise AgentError("catalog_not_found", "Catalog does not exist", status_code=404)
            return catalog
        if mode == "append":
            raise AgentError("missing_catalog", "Append requires an existing catalog")
        catalog = Catalog(owner_id=owner_id, name=catalog_name.strip())
        session.add(catalog)
        session.flush()
        return catalog
    ...
    def _materialize(
        ...
        if mode == "append":
            with Session(self.engine) as session:
                parent = session.get(CatalogVersion, parent_version_id)
                if parent is None or parent.catalog_id != catalog_id or parent.owner_id != owner_id:
                    raise AgentError(
                        "invalid_parent", "Append parent does not belong to the catalog"
                    )
    ...
    def _open_verified_blob(self, blob: Blob) -> Iterator[BinaryIO]:
        try:
            ...
            raise AgentError(
                "managed_blob_missing", "Managed catalog blob is missing", status_code=500
            ) from exc
        try:
    ...
    def materialized_path(
        ...
        if version is None or version.owner_id != owner_id:
            raise AgentError(
                "catalog_version_not_found", "Catalog version does not exist", status_code=404
            )
    ...
    def create_mapping(
        ...
        with session_scope(self.engine) as session:
            ...
            row_id = row.id
        with Session(self.engine) as session:
            return session.get_one(CatalogMapping, row_id)
...
def compute_profile(
    ...
    with Session(registry.engine) as session:
        mapping_row = session.get(CatalogMapping, mapping_id)
        if (
            mapping_row is None
            or mapping_row.catalog_version_id != version_id
            or mapping_row.owner_id != owner_id
        ):
            raise AgentError("mapping_not_found", "Mapping does not belong to the catalog version")
...
def publish_profile(
    ...
    row = DataProfile(
        ...
        catalog_version_id=computation.catalog_version_id,
