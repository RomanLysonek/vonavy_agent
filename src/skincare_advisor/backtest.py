...
from skincare_advisor.managed_files import fsync_tree
...
def _ridge_design(
    ...
    recommendation_date = date.fromordinal(origin.toordinal() + horizon - 1)
...
def _baseline_predictions(
    ...
    for entity in entities:
        if isinstance(config, SeasonalNaiveConfig):
            reference = date.fromordinal(recommendation_date.toordinal() - config.period_days)
...
def run_backtest(
    ...
    with Session(engine) as session:
        ...
        mapping_row = session.get_one(CatalogMapping, spec_row.mapping_id)
        if (
        ...
        for seed in spec.seeds:
            for model in spec.models:
                for horizon in range(1, spec.horizon_days + 1):
                    ...
                    recommendation_date = date.fromordinal(
                        origin_spec.date.toordinal() + horizon - 1
                    )
                    for entity, prediction in model_predictions.items():
                        actual_row = lookup.get((entity, recommendation_date))
    ...
    manifest = {
        ...
        "catalog_hash": _catalog_hash(engine, spec.catalog_version_id, owner_id),
        ...
        "command": [sys.executable, "-m", "skincare_advisor.executor", "--job-id", job_id],
...
def _catalog_hash(
    ...
    from skincare_advisor.persistence import CatalogVersion
...
def _environment(settings: Settings) -> dict[str, Any]:
    ...
    for name in ("skincare-advisor", "pandas", "numpy", "pyarrow", "scikit-learn"):
        try:
