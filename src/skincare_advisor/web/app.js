...
$("upload-form").addEventListener("submit",async event=>{
  event.preventDefault();notice("Copying and hashing catalog…");
  try{
    const data=new FormData();data.append("catalog_name",$("catalog-name").value);data.append("file",$("catalog-file").files[0]);
    state.version=await api("/api/catalogs/upload",{method:"POST",body:data});
    show("catalog-result",`Version ${state.version.version_number} / ${state.version.row_count} rows / SHA-256 ${state.version.materialized_sha256}`);
    notice("Immutable catalog version created. Define the mapping.");
  }catch(error){notice(error.message,true)}
});

$("mapping-form").addEventListener("submit",async event=>{
  event.preventDefault();if(!state.version){notice("Ingest a catalog first.",true);return}
  ...
  try{
    ...
    state.mapping=await api(`/api/catalog-versions/${state.version.id}/mappings`,jsonOptions(mapping));
    const queued=await api("/api/profiles",jsonOptions({catalog_version_id:state.version.id,mapping_id:state.mapping.id}));
...
$("spec-form").addEventListener("submit",async event=>{
  event.preventDefault();if(!state.profile){notice("Profile the mapped catalog first.",true);return}
  ...
  try{
    ...
    const spec={
      schema_version:"1.0",catalog_version_id:state.version.id,mapping_id:state.mapping.id,profile_id:state.profile.id,frequency:"D",
