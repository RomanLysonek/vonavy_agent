...
from skincare_advisor.api import create_app, migrate
from skincare_advisor.jobs import Worker
from skincare_advisor.persistence import create_db_engine
...
def demo_data(destination: Path) -> None:
    ...
    for entity_index, entity in enumerate(("product_id-a", "product_id-b")):
        for offset in range(210):
            ...
            weekday = day.weekday()
            skin_type_match = int((offset + entity_index) % 17 == 0)
            rating = (
                90
                + entity_index * 20
                + weekday * 3
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="skincare-advisor")
