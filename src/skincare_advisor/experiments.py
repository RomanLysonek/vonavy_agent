...
from skincare_advisor.catalogs import CatalogRegistry, observation_availability
...
from skincare_advisor.eligibility import expected_grid
from skincare_advisor.errors import AgentError
from skincare_advisor.hashing import canonical_hash, canonical_json
...
from skincare_advisor.policy import ResourcePolicy
...
def create_experiment_spec(
    ...
    with session_scope(engine) as session:
        ...
        mapping = session.get(CatalogMapping, spec.mapping_id)
        version = session.get(CatalogVersion, spec.catalog_version_id)
        if (
            profile is None
            or mapping is None
            or version is None
        ...
        if (
            profile.catalog_version_id != spec.catalog_version_id
            or mapping.catalog_version_id != spec.catalog_version_id
            or profile.mapping_id != spec.mapping_id
        ):
            raise AgentError("evidence_mismatch", "Catalog, mapping, and profile do not match")
        row = ExperimentSpecRow(
            owner_id=owner_id,
            spec_hash=canonical_hash(payload),
            canonical_json=canonical_json(payload),
            catalog_version_id=spec.catalog_version_id,
...
def compute_gate(
    ...
    with Session(engine) as session:
        ...
        mapping_row = session.get_one(CatalogMapping, spec.mapping_id)
    ...
    if profile["rows"] > spec.resources.max_rows:
        block("row_limit", "Catalog row count exceeds the resource limit", profile["rows"])
    if profile["entities"] > spec.resources.max_entities:
        block(
            "entity_limit", "Catalog entity count exceeds the resource limit", profile["entities"]
        )
    ...
    if profile["duplicate_key_rows"]:
        block(
            ...
            remediation="Create a new catalog version with unique entity/date rows.",
        )
    if profile["gap_days"]:
        block(
            "daily_gaps",
            ...
            profile["gap_days"],
            remediation="Create a complete daily catalog version or shorten the explicit split.",
        )
    ...
    for origin in spec.origins:
        ...
        for entity in entity_values:
            ...
            for horizon in range(1, spec.horizon_days + 1):
                recommendation_date = pd.Timestamp.fromordinal(
                    origin.date.toordinal() + horizon - 1
                )
                row_mask = entity_mask & (dates == recommendation_date)
                ...
                if spec.scoring_availability_policy == "require_available" and not row_observed:
                    ...
                    required_unavailable_examples.append(
                        f"{entity}@{recommendation_date.date().isoformat()}"
                    )
                if (
                    int(row_mask.sum()) == 1
                    and row_observed
    ...
    from skincare_advisor.backtest import _prepare_data, model_feasibility
    ...
    if coverage < spec.minimum_coverage:
        block(
            ...
            remediation="Adjust evaluation_as_of, coverage threshold, origins, or the catalog version.",
        )
