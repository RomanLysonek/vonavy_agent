...
class DeploymentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.max_catalogs_per_owner <= 1000:
            raise ValueError("max_catalogs_per_owner must be between 1 and 1000")
        minimum_hard_quota = self.max_upload_bytes * self.max_catalogs_per_owner
    ...
    def __init__(
        ...
        *,
        ...
        web_security_headers = cloudfront.ResponseHeadersPolicy(
            self,
            "WebSecurityHeaders",
            comment="Security headers for the skincare-advisor static application",
        ...
        api_resource_scope = cognito.ResourceServerScope(
            scope_name="api",
            scope_description="Use the skincare-advisor control-plane API",
        )
        resource_server = user_pool.add_resource_server(
            "ResourceServer",
            identifier="skincare-advisor",
        ...
        domain_prefix = f"skincare-{config.environment_name}-{Aws.ACCOUNT_ID}"
        ...
        control_plane_function = lambda_.Function(
            ...
            environment={
                ...
                "MAX_DATASETS_PER_OWNER": str(config.max_catalogs_per_owner),
        ...
        control_plane_function.add_to_role_policy(
            iam.PolicyStatement(
                ...
                resources=[data_bucket.arn_for_objects("catalogs/users/*")],
            )
        )
        ...
        for path, method in (
            ...
            ("/api/catalogs", apigwv2.HttpMethod.GET),
        ):
            http_api.add_routes(
                path=path,
                methods=[method],
                ...
                authorization_scopes=["skincare-advisor/api"],
            )
            ...
            sources=[
                ...
                s3deploy.Source.json_data(
                    "config.json",
                    {
                        ...
                        "redirectUri": web_url,
                        "scope": "openid email skincare-advisor/api",
                        "maximumUploadBytes": config.max_upload_bytes,
                        "maximumCatalogsPerOwner": config.max_catalogs_per_owner,
