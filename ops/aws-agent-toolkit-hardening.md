...
the `skincare-admin` AdministratorAccess SSO profile.

## Required identity separation

Create an IAM Identity Center permission set named `SkincareReadOnly` and attach
the AWS managed `ReadOnlyAccess` policy. Assign it only to Roman's normal Identity Center user in the target account
returned by `aws sts get-caller-identity --profile skincare-admin`.

Create a separate CLI profile:

```bash
aws configure sso --profile skincare-readonly
aws sso login --profile skincare-readonly
aws sts get-caller-identity --profile skincare-readonly
```

Verify that its ARN is an assumed Identity Center role for `SkincareReadOnly`, not
...
Do not add `skincare-admin` as another `--profile` value or through
...
Use `skincare-admin` only in an explicit terminal command after Roman approves the
