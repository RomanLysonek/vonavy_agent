...
class CatalogMappingSpec(StrictModel):
    ...
    @model_validator(mode="after")
    def unique_columns(self) -> CatalogMappingSpec:
...
class EvaluationSpec(StrictModel):
    ...
    def validate_boundaries(self) -> EvaluationSpec:
        ...
        if not self.origins:
            raise ValueError("at least one recommendation origin is required")
        origin_keys = [(origin.date, origin.role) for origin in self.origins]
        if len(origin_keys) != len(set(origin_keys)):
            raise ValueError("recommendation origins must be unique")
...
class RecommendationSpec(StrictModel):
    schema_version: Literal["1.0"] = "1.0"
    ...
    @model_validator(mode="after")
    def validate_recommendation_contract(self) -> RecommendationSpec:
        if self.training_end >= self.recommendation.start:
            raise ValueError("recommendation dates must start after the final training date")
        if self.horizon_days > 90:
            raise ValueError("recommendation horizon must not exceed 90 days")
        if self.information_cutoff.date() > self.recommendation.start:
            raise ValueError("information cutoff must not be after the first recommendation date")
        if not self.models:
            raise ValueError("at least one model is required")
        if len(self.models) > self.resources.max_models:
            raise ValueError("model count exceeds resource limit")
    ...
    def horizon_days(self) -> int:
        return (self.recommendation.end - self.recommendation.start).days + 1
...
RunSpec: TypeAlias = Annotated[
    EvaluationSpec | RecommendationSpec | InferenceSpec,
