...
from skincare_advisor.domain import ExperimentSpec, StrictModel
...
from skincare_advisor.managed_files import publish_bytes
from skincare_advisor.persistence import AdapterSnapshot, session_scope
...
class CapabilityManifest(StrictModel):
    ...
    adapter_kind: Literal["anomaly", "optional_model"]
    adapter_version: str
    available: bool
...
def adapter_capabilities(
    ...
    defaults: dict[str, dict[str, object]] = {
        ...
        "optional_model": CapabilityManifest(
            adapter_kind="optional_model",
            adapter_version="unconfigured",
            available=False,
            unavailable_reason="No validated optional model capability snapshot has been imported",
        ).model_dump(mode="json"),
    }
    with Session(engine) as session:
        rows = (
