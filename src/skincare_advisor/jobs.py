...
from skincare_advisor.hashing import canonical_json, file_hash
from skincare_advisor.identity import LOCAL_OWNER_ID
from skincare_advisor.managed_files import verified_managed_file, verify_run_bundle
...
class Worker:
    ...
    def _finish_success(self, claim: Claim) -> None:
        try:
            ...
            self._finish_failed(
                claim,
                "artifact_skin_type_match_failed",
                str(exc),
            )
            return
        with session_scope(self.engine) as session:
    ...
    def _record_terminal_run(self, job_id: str, status: str, error: dict[str, Any]) -> None:
        from skincare_advisor.backtest import _environment, _source_revision
        ...
        with Session(self.engine) as session:
            ...
            mapping = session.get_one(CatalogMapping, spec.mapping_id)
            catalog = session.get_one(CatalogVersion, spec.catalog_version_id)
        ...
        if not (final_dir / "manifest.json").is_file():
            ...
            manifest = {
                ...
                "catalog_hash": catalog.materialized_blob_sha256,
