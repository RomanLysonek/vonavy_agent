...
from skincare_advisor.adapters import (
    PreparedInvocation,
    adapter_capabilities,
    dry_run_invocation,
    import_adapter_snapshot,
)
from skincare_advisor.catalogs import CatalogRegistry
from skincare_advisor.domain import CatalogMappingSpec, ExperimentSpec, JobState
from skincare_advisor.errors import AgentError
from skincare_advisor.experiments import create_experiment_spec
from skincare_advisor.identity import IdentityContext, IdentityProvider, LocalIdentityProvider
...
def create_app(
    ...
    registry = CatalogRegistry(settings, engine)
    ...
    app = FastAPI(title="Skincare Advisor", version="0.2.0", lifespan=lifespan)
    ...
    def list_catalogs(identity: Identity) -> dict[str, object]:
        with Session(engine) as session:
            catalogs = session.scalars(
                select(Catalog)
                .where(Catalog.owner_id == identity.owner_id)
                .order_by(Catalog.created_at)
            ).all()
            versions = session.scalars(
                select(CatalogVersion)
                .where(CatalogVersion.owner_id == identity.owner_id)
                .order_by(CatalogVersion.catalog_id, CatalogVersion.version_number)
            ).all()
        by_catalog: dict[str, list[dict[str, object]]] = {}
        for version in versions:
            by_catalog.setdefault(version.catalog_id, []).append(_version_json(version))
        return {
            "catalogs": [
                {
                    "id": catalog.id,
                    ...
                    "versions": by_catalog.get(catalog.id, []),
                }
                for catalog in catalogs
            ]
        }

    @app.get("/api/catalog-versions/{version_id}")
    ...
    @app.post("/api/catalog-versions/{version_id}/mappings")
    def create_mapping(
        version_id: str,
    ...
    def adapter_dry_run(adapter_kind: str, identity: Identity) -> dict[str, object]:
        del identity
        if adapter_kind not in {"anomaly", "optional_model"}:
            raise AgentError("adapter_unknown", "Unknown adapter kind")
        cwd = (settings.managed_root / "imports").resolve()
        invocation = PreparedInvocation(
            executable=sys.executable,
            argv=("-m", f"skincare_advisor.adapters_{adapter_kind}", "--capabilities"),
            cwd=str(cwd),
            timeout_seconds=30,
            expected_outputs=(),
        )
...
def _version_json(version: CatalogVersion) -> dict[str, object]:
    return {
        "id": version.id,
